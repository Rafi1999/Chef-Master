...
const Header = () => {
    ...
    return (
        ...
        {user ? <div className='flex gap-2'><img className='rounded-lg w-10' src={user.photoURL} onMouseEnter={()=>setShow(true)} onMouseLeave={()=>setShow(false)}></img>{show && <p className='relative top-2'>{user.displayName}</p>}<button onClick={logOut}>LogOut</button> </div> : <button><NavLink to='/login' className='hover:text-red-500'>Login</NavLink></button>}
